...
class UserTenantRepository(IUserTenantRepository):
    async def list(self) -> Sequence[UserTenant]:
        stmt = select(UserTenant)  # type: ignore

        result = await self._session.execute(stmt)

        usertenants = result.scalars().all()
    ...
    async def get_by_id(self, id: UUID) -> UserTenant | None:
